...
def get_brand_category(text):
    ...
    brand_map = {
        ...
        r'@idea_cares|\bidea cellular\b': ('Idea', 'Telecom & Internet'),
...
# --- STREAMING ---
df_sales = spark.readStream.format("kafka").option("kafka.bootstrap.servers", "kafka:9092").option("subscribe", "superstore_sales").option("startingOffsets", "earliest").load()
...
# --- TRANSFORM ---
sales_schema = StructType().add("Order ID", StringType()).add("Order Date", StringType()).add("Customer Name", StringType()).add("Category", StringType()).add("Sales", FloatType()).add("Quantity", IntegerType()).add("Profit", FloatType()).add("State", StringType())
